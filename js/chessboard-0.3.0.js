...
function validSquare(square) {
  ...
  if (square.search(/^[a-n][1-9][0-4]?$/) === -1) return false;
  if (square.search(/^[abclmn][123][2-4]?$/) !== -1) return false;
  return true;
}
...
function expandConfig() {
  ...
  if (cfg.hasOwnProperty('position') == true) {
    if (validPositionObject(cfg.position) === true) {
...
widget.validMove = function(move) {
...
widget.changePlayerPiecesColorNoKing = function(player, color) {
  for (var i in CURRENT_POSITION) {
    if (CURRENT_POSITION[i][0] === player && CURRENT_POSITION[i][1] != 'K') CURRENT_POSITION[i] = color + CURRENT_POSITION[i][1];
  }
  drawPositionInstant();
}
